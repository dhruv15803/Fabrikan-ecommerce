    ...
    try {
        ...
        for (let i = 0; i < cartItems.length; i++) {
            if (String(cartItems[i].productId) === String(product._id)) {
                ...
                if (attributes.length !== dbAttributes.length)
                    continue;
                ...
                if (isAttributesDiff)
                    continue;
                isDuplicate = true;
                newCartItem = await CartItem.findByIdAndUpdate(cartItems[i]._id, { $set: { cartItemQty: cartItems[i].cartItemQty + 1, cartItemAttributes: attributes } }, { new: true }).populate('productId');
                break;
            }
        }
        if (!isDuplicate) {
        ...
        return res.status(200).json({ success: true, message: "Added cart item", newCartItem });
    }
    catch (error) {
...
const decrementItemQty = async (req, res) => {
    try {
        ...
        if (cartItem.cartItemQty <= 1)
            return res.status(400).json({ "success": false, "message": "item qty already at 1" });
...
export { addCartItem, removeCartItem, incrementItemQty, decrementItemQty, };
